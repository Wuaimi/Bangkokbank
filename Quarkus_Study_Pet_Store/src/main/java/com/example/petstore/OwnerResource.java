...
public class OwnerResource {
    ...
    @APIResponse(responseCode = "200", description = "Delete Pet from Owner", 
        content = @Content(schema = @Schema(implementation = Owner.class)))
    public Response deletePetFromOwner(
        @PathParam("petId") Integer petId,
        @RequestBody(
            description = "Owner ID to delete pet from",
            required = true,
            content = @Content(
                mediaType = "application/json",
                examples = @ExampleObject(
                    name = "DeletePetFromOwnerExample",
                    summary = "Example delete pet from owner",
                    value = "{ \"ownerId\": 123 }"
                )
            )
        ) Owner owner) {

        boolean removed = ownerList.removePetFromOwner(petId, owner.getOwnerId());
        if (removed) {
            Owner updatedOwner = ownerList.getOwnerById(owner.getOwnerId());
            return Response.ok(updatedOwner).build();
        }
        return Response.status(Response.Status.NOT_FOUND).entity("Owner or Pet not found").build();
    }
